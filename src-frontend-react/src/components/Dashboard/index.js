      ...
      customFetch(`${global.server_backend_url}/backend/appointments/staff-availability`)
        ...
        .then((data) => {
          data = data
            ...
            .map((x) => {
              x.dateString = `${moment(x.fromDate).format('dddd')}, ${moment(x.fromDate).format('hh:mm A')} to ${moment(x.toDate).format('hh:mm A')}`
              return x
            })
  ...
  useEffect(() => {
    ...
    setAvailableSchedules(
      cookies.usertype === user_type.Student
        ? staffAvailability
          .filter((appointment) => searchText.trim().length === 0 || Object.values(appointment).some(value => {
            delete value.id
        ...
        : appointments.filter((appointment) => appointment.state === schedule_state.Available)
          .filter((appointment) =>
          ...
          <Container>
                              ...
                              {nonAvailableSchedules.map(appointment => (
                                <tr>
                                  <td>
                                    <Nav.Link
                                      as={Link}
                                      to={`/appointments?datetime=${moment(appointment.fromDate).toISOString()}`}>{appointment.title}
                                    </Nav.Link>
                                  </td>
            ...
            <Row className="py-2">
              <Col>
                <Card className="shadow-sm">
                  <Card.Header as={"h2"}>
                    {cookies.usertype === user_type.Student ? 'Staff Availability' : 'My Availability'}
                  </Card.Header>
                  <Card.Body className="d-flex justify-content-center">
                    {
                        ...
                        >
                          <Table className="appointments-table w-100">
                            <thead>
                              <tr className="text-uppercase">
                                <th scope="col">Title</th>
                                {cookies.usertype === user_type.Student ? <th scope="col">By Staff</th> : ''}
                            ...
                            <tbody>
                              {availableSchedules.map(appointment => (
                                <tr>
                                  ...
                                  {cookies.usertype === user_type.Student ? <td>{`${appointment.authoredBy.lname}, ${appointment.authoredBy.fname}`}</td> : ''}
