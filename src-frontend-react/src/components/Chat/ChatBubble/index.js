...
const Chat = ({ scheduleId, hideTextBox = false }) => {
  ...
  const fetchAll = useCallback(() => {
    ...
    if (scheduleId)
      customFetch(
        `${global.server_backend_url}/backend/appointments/messages/by-schedule/${scheduleId}`,
      )
        .then((response) => {
          if (response.ok) {
  ...
  return (
    <LoadingOverlay spinner active={isLoading}>
      <Container className="px-3 mb-4">
        <div
          className="overflow-scroll p-3 mb-4 d-flex flex-column-reverse"
          style={
            hideTextBox
              ? {}
              : { maxHeight: "30rem", borderBottom: "1px solid #ccc" }
          }
        >
          {messages.map((message, index) => (
            <div
              key={index}
              className={`my-2 ${
                message.User.login_username === cookies.login_username
                  ? "text-end"
                  : "text-start"
              }`}
            >
              <Card
                className="d-inline-block text-break m-1 px-1 rounded-3 shadow-sm"
              ...
              >
                <Card.Body>
                  ...
                  {message.content}
                  <p
                    className={`mt-1 mb-0 text-black-50 ${
                      message.User.login_username === cookies.login_username
                        ? "text-end"
                        : "text-start"
                    }`}
